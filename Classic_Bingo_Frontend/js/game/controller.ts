...
import { GameSessionData, GameConfig, WinnerInfo, DaubResponse } from './types';
import { Storage } from '../storage';

export class GameController {
    private gameUI: GameUI;
    private numberCallInterval: number | null = null;
    private sessionData: GameSessionData | null = null;
    ...
    async startMultiplayerGame(sessionId: string): Promise<void> {
        try {
    ...
    private startNumberCallingLoop(): void {
        if (!this.sessionData) return;
    ...
    private stopNumberCallingLoop(): void {
    ...
    private async fetchNextNumber(): Promise<void> {
    ...
    try {
        ...
        if (response.success) {
            ...
            if (response.data.autoDaub && Array.isArray(response.data.autoDaub)) {
                ...
                response.data.autoDaub.forEach((daub: { cardIndex: number; cellIndex: number }) => {
...
private async handleCellClick(event: Event): Promise<void> {
    ...
    try {
        const response: DaubResponse = await GameAPI.daubNumber(
            this.sessionData.sessionId,
            daubedNumber,
            cardId // Send actual server cardId
        );
...
private async handleBingoClaim(cardId: number): Promise<void> {
        if (!this.sessionData) return;
        ...
        try {
            ...
            if (response.success || response.data?.claimValid) {
                const winners: WinnerInfo[] = 
                    ...
                    (response as any).winners ||
                    [];

                this.handleGameOver(winners);
            } else {
                this.gameUI.showWarningModal(
                    response.message || 'No winning pattern detected on this card',
                    () => console.log('[GAME] Continuing game after invalid bingo')
                );
            }
        } catch (error: any) {
            console.error('[GAME] Failed to claim bingo:', error);
            this.gameUI.showWarningModal(
                error.message || 'Failed to verify bingo claim',
                () => console.log('[GAME] Continuing game')
            );
        }
    }

    private async handleGameOver(winners: WinnerInfo[]): Promise<void> {
    if (!this.sessionData) return;
    ...
    const isWin = winners.some(winner => 
        winner.userId === currentUserId && winner.type === 'user'
    );
