...
import { BingoCard, WinnerInfo } from './types';
...
export class GameUI {
    ...
    private cardComponent: BingoCardComponent;
    private modals: GameModals;
    ...
    renderGameScreen(cards: BingoCard[], calledNumbers: number[]): void {
    ...
    updateNumberDisplay(numbers: number[]): void {
    ...
    highlightCell(cardId: number, cellIndex: number): void {
    this.cardComponent.highlightCell(cardId, cellIndex);
}

    showResultModal(isWin: boolean, winners: WinnerInfo[]): void {
        this.modals.showResultModal(isWin, winners, () => {
            window.location.href = '/index.html';
        });
    }

    showWarningModal(message: string, onContinue: () => void): void {
