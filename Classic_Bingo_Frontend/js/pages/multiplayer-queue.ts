...
export class MultiplayerQueuePage {
    ...
    private statusPolling: number | null = null;

    async init(): Promise<void> {
        this.showCardSelection();
    }

    private showCardSelection(): void {
    ...
    private async joinQueue(numberOfCards: number): Promise<void> {
        try {
            // Show loading state
            const btn = document.getElementById('join-queue-btn') as HTMLButtonElement;
            if (btn) {
                btn.disabled = true;
                    ...
                    if (participants.length === 0) {
                        ...
                        playersList.innerHTML = `
                            ...
                            ${participants.map((p: string, index: number) => 
                                `<div class="player-item">Player ${index + 1}</div>`
                            ).join('')}
                        `;
                    }
                }
