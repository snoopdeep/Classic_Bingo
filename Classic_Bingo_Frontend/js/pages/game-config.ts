...
import { GameMode, GameConfig, WinningPattern, BallSpeed } from '../game/types';

export class GameConfigPage {
    private selectedMode!: GameMode;
    private aiOpponents: number = 1;
    private userCards: number = 1;
    private aiCardCounts: number[] = [1];

    private practiceWinningPattern: WinningPattern = 'standard';
    private practiceBallSpeed: BallSpeed = 'normal';
    private practiceAutoDaub: boolean = false;

    async init(): Promise<void> {
        const mode = sessionStorage.getItem('selectedGameMode') as GameMode;
        if (!mode) {
    ...
    private initializePracticeControls(): void {
        // Set initial selected pattern
        (document.getElementById('winning-pattern') as HTMLSelectElement).value = this.practiceWinningPattern;
        // Set initial auto-daub state
        (document.getElementById('auto-daub-toggle') as HTMLInputElement).checked = this.practiceAutoDaub;
...
private renderSoloBanner(): string {
    return `
...
private renderSoloConfig(): string {
    return `
    ...
    private renderPracticeBanner(): string {
        return `
            <div class="practice-banner">
                <p>
    ...
    private renderPracticeConfig(): string {
        return `
            <div class="config-section">
                <label class="config-label">
                    🎯 Winning Pattern
    ...
    private renderSpeedButton(speed: BallSpeed, label: string, time: string): string {
   ...
   private getModeTitle(): string {
    ...
    private renderBasicConfig(): string {
        return `
            <div class="config-section">
    ...
    private renderAICardsList(): string {
    ...
    private attachPracticeListeners(): void {
        ...
        const patternSelect = document.getElementById('winning-pattern') as HTMLSelectElement;
        if (patternSelect) {
            patternSelect.addEventListener('change', (e) => {
                this.practiceWinningPattern = (e.target as HTMLSelectElement).value as WinningPattern;
            });
            // Ensure the initial value is set for the TS property
            this.practiceWinningPattern = patternSelect.value as WinningPattern;
        }
        ...
        document.querySelectorAll('.speed-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                const speed = target.dataset.speed as BallSpeed;
        ...
        // Auto-daub toggle
        const autoDaubToggle = document.getElementById('auto-daub-toggle') as HTMLInputElement;
        if (autoDaubToggle) {
            autoDaubToggle.addEventListener('change', (e) => {
                this.practiceAutoDaub = (e.target as HTMLInputElement).checked;
            });
             // Ensure the initial value is set for the TS property
    ...
    private attachAICardListeners(): void {
    ...
    private updateUI(): void {
        ...
        if (aiCountEl) aiCountEl.textContent = this.aiOpponents.toString();
        this.updateAICardsList();
    }

    private updateUserCount(): void {
        const userCountEl = document.getElementById('user-count');
        if (userCountEl) userCountEl.textContent = this.userCards.toString();
    }

    private updateAICardsList(): void {
